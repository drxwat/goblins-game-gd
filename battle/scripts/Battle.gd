...
#			battleUI.hide_enemy_info()
		...
		if selected_unit != null:
			_draw_trace_path(
				selected_unit,
				selected_unit.global_transform.origin,
				m_position
			)
...
func _draw_trace_path(unit: BattleUnit, from: Vector3, to: Vector3):
	...
	for point in path_points:
		...
		dot.set_path_color(
				_get_current_trace_path_color(
					unit, 
					unit.max_move_points - unit.move_points + i + 1
				)
		)
...
func _move_mouse_hover(pos: Vector3):
	var sum = terrain.get_map_cell_center(pos) + MOUSE_HOVER_Y_OFFSET
	mouse_hover.translation = sum
...
func _occupy_enemy_unit_point():
	if hovered_enemy:
		terrain.occupy_point_with_unit(
					hovered_enemy.global_transform.origin,
					hovered_enemy.battle_id
		)
		hovered_enemy = null
